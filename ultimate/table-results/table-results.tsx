...
export interface TableResultsProps extends WithTranslation, RouteComponentProps, CacheProps, IdentityProps, UserProfileProps {
  grants: any;
}

const TableResults: React.FC<TableResultsProps> = (props) => {
  const { t, history, grants } = props;
  ...
  return <Table rowKey={(record: any) => record.id} columns={columns} dataSource={grants} pagination={false} size="middle" />;
};

export default withIdentity(withUserProfile(withCache(withTranslation()(withRouter(TableResults)))));
